...
/**
 * @description: 在滑动窗口下，测试点击流中重复点击入口
...
public class Main
{
    ...
    {
        System.out.println("Yahoo data test Startup: c="+c+"  m="+m+"  k="+k);
        int line=0;      //记录总数
        int numberOfDatasets = 2;                       //所使用数据集的个数
        String datasetName = ClickStreamCfg.FILE_PATH;
        ...
        {
            datasetName = ClickStreamCfg.FILE_PATH;
            if (datasetId < 10)
            ...
            try
            {
                while((str=buffer.readLine())!=null){
                    ...
                    DuplicateAlgorithm(element,line);
                    line++;
                }
            }
            catch (IOException e)
        ...
        System.out.println("Yahoo data test End: c="+c+"  m="+m+"  k="+k);
